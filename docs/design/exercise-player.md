...
* ~~Observer player variants (team / station perspectives) and the perspective pill.~~ **Delivered, in a generalized form — see [ADR-0056](../adrs/0056-player-modes-exercise-station-roleplay.md).** Rather than separate observer *players* with their own template, exercise / station / **roleplay** are peer **modes** of the one player: `_DrillPlayerHost` renders any `ContextSheetTarget` via `defaultContextSheetBody`, so each mode is the same `StationScreen` / `RolePlayScreen` the rest of the app shows. The perspective pill's job is done by the mini bar's badge, which follows the mode and whose picker lists siblings of that mode's type. The team variant is *not* delivered: a team is not a player mode, so team taps still open `TeamScreen` / `TeamExerciseScreen` outside the player.
...
* [ADR-0056: One drill player with three peer modes — exercise, station, roleplay](../adrs/0056-player-modes-exercise-station-roleplay.md) — delivers the parked observer-player variants as modes of the single player instead of separate player shells.
...
* 2026-07-27 — The parked observer-player variants are delivered in generalized form: exercise, station and roleplay are peer **modes** of the one player rather than separate players, with the mini bar's mode-following badge in place of the perspective pill. The team variant is deliberately not delivered — a team is not a player mode. See [ADR-0056](../adrs/0056-player-modes-exercise-station-roleplay.md).
