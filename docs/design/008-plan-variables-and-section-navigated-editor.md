...
**Follow-up 12 (2026-07-09) — Section-editor polish (override list + wide chrome).** Two UI refinements to the section-navigated editor, no model change. **(a) Variable override list.** The exercise/station override surface becomes card-per-item, like the DESIGN-009 Persons/Locations lists: each card is the variable name (mono) with its inherited default in parentheses (`year (2026)` — no "Arvet" label), a local-value field below (placeholder "Lokal verdi" when empty), and an accent border plus a per-variable "Tilbakestill" when overridden. **(b) Wide master/detail chrome** ([ADR-0030](../adrs/0030-wide-screen-master-detail-layout.md)): drop the duplicated section title in the detail pane and the redundant `‹ ›` arrows; the `⋮` section-actions (remove) menu is **overlaid** in the detail pane's top-right corner and shown **only on a removable** section — a base section (Plan, Variabler, Personer, Lokasjoner) gets no header at all, and the overlay costs no row. The duplicated body title turned out to be the markdown section field's own floating label (`RingDrillTextArea(label: <section name>)`), which repeats the switcher's name; it is dropped (the field takes no label, `RingDrillTextArea.label` made optional), since the compact dropdown / wide rail already names the section; reuse the compact layout's bottom search / "+ Ny …" row (a rounded elevated `Card`, as shipped, with an accent-text add action) — no new treatment; the current wide layout only double-inset it in an extra outer margin, so removing that padding lets the Card span the detail pane as it does on compact; and put the left rail on the same surface palette as the rest (not near-black), with the selected section accent-highlighted like the compact switcher. Compact is unchanged. Mockups: `mockups/variable-overrides.html`, `mockups/post-editor-wide.html`. Prompt: `docs/prompts/design-008-12-section-editor-polish.md`.
