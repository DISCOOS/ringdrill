...
4. ~~**Example plan delivery.**~~ **Resolved (2026-06-07):** bundled localized `.drill` asset under `assets/example/`, loaded via `rootBundle` and installed with `ProgramService.installFromFile(activate: true)`. Simpler and offline by default, over a built-in catalog entry ([ADR-0008](../adrs/0008-persistent-program-library-and-catalog.md)) or an in-Dart factory. Authored by extending the deterministic generator behind the screenshot demos (`tools/screenshots/make_demo_drills.py`) to emit a small current-schema example per locale. The existing `demo-no.drill` / `demo-en.drill` are *not* reused — they are screenshot fixtures (five exercises, four teams, schema 1.0, no markører, no brief), the wrong shape for onboarding.
...
* 2026-06-07 — Resolved deferred decision 4 (example plan delivery): bundled localized `.drill` asset under `assets/example/`, installed via `ProgramService.installFromFile(activate: true)`, authored by extending the screenshot-demo generator to current schema. The existing `demo-no/en.drill` screenshot fixtures are not reused (wrong shape and schema). Stage 3 prompt drafted.
