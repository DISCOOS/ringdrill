...
> **Implementation note (2026-07-02).** `/catalog` (under [Site source](#site-source) below) renders on-demand, not as a build-time-static page as originally sketched here. It uses the `@astrojs/cloudflare` adapter (mixed static/on-demand output — every other page stays prerendered) and fetches `GET /api/market-feed` at request time from a configurable `PUBLIC_RINGDRILL_API_BASE`, following the same rendering pattern designed for ADR-0044's `/i/[slug]` route. `/catalog` is the first route to introduce this adapter and env-base convention; ADR-0044's own "add the adapter" step becomes a no-op confirmation when it is implemented. This also makes the hourly `schedule` cron in `deploy-origins.yml` (added to keep a build-time catalog index fresh) no longer load-bearing for `/catalog` specifically, since the page is never stale by more than its own edge-cache window — the cron is left in place rather than removed as part of this change.

> **Implementation note (2026-07-19).** The apex **site** now deploys as a **Cloudflare Worker** (`ringdrill-site`), not Cloudflare Pages: `@astrojs/cloudflare` v13+ emits a Workers bundle rather than a Pages one, so the same-named Pages project is retired. This further relaxes the "pure static, no Workers" driver below (already relaxed by the apex-proxy Worker in the 2026-07-02 note). The three origins are now: site = Cloudflare Worker (`ringdrill-site`, apex), PWA = Cloudflare Pages (`ringdrill-pwa`, `web.`), API = Netlify functions (`api.`); plus the standalone apex-proxy Worker. All deploy from GitHub Actions (`deploy-origins.yml`, `deploy-proxy.yml`). See [`docs/api.md`](../api.md) for the current topology.
...
## Addendum (2026-07-02): `site` origin moves from Cloudflare Pages to Cloudflare Workers

This ADR originally put `ringdrill.app` on Cloudflare Pages (project `ringdrill-site`), matching `web.ringdrill.app`'s `ringdrill-pwa` Pages project. That changed when the `/catalog` route needed on-demand rendering (ADR-0044's pattern, applied ahead of ADR-0044 itself landing): `@astrojs/cloudflare` v13+ dropped Pages Functions support and only targets Cloudflare Workers — the adapter's own README changed from "Cloudflare Pages Functions targets" to "Cloudflare Workers targets" partway through the v13 line, and the last Pages-capable release (v12.6.13) requires `astro: ^5.7.0`, two majors behind the `astro: ^7.0.4` the rest of the site already depends on. Downgrading Astro just to keep the old deploy path was rejected as by far the more expensive path.

**What changed.** `site/` now builds to a Workers-with-assets bundle (`dist/client` static assets + `dist/server/{entry.mjs,wrangler.json}`), deployed with `wrangler deploy --config dist/server/wrangler.json` from a Worker named `ringdrill-site`, not `wrangler pages deploy`. The `ringdrill.app` custom domain moved from the old `ringdrill-site` Pages project to this Worker. The old Pages project is retired — nothing deploys to it anymore, so nothing should point at it (see the `www` note below).

**`site/wrangler.json`.** A minimal project-root config (`{"assets": {"run_worker_first": [...]}}`) that the adapter merges into its generated `dist/server/wrangler.json`. Needed because Cloudflare's default `not_found_handling` for Workers-with-assets serves the static `404.html` directly for any path with no matching asset file — correct for a real 404, wrong for an on-demand route like `/catalog` that has no static file by design. `run_worker_first` is scoped to exactly `["/catalog", "/en/catalog"]` rather than `true`: a blanket `true` also forces `astro dev`'s own internal asset URLs (`/@vite/client`, `*.astro?astro&type=...`) through the simulated Worker in local dev, breaking the dev server entirely. Scoping to the actual on-demand routes fixes production without touching anything else.

**`www.ringdrill.app`.** Never one of this ADR's three origins — an incidental CNAME to the old Pages project, not a distinct served surface. Rather than give it its own Workers custom domain (a second thing to keep in sync with the apex forever), it now redirects to `ringdrill.app` via a Cloudflare Redirect Rule. This has zero maintenance cost and can never drift from what the apex actually serves.

**Consequences.** `CLOUDFLARE_API_TOKEN` needs Workers Scripts + Workers KV Storage edit scopes in addition to whatever Pages scope the `pwa` job still needs (that one stays on Pages — it's a plain static Flutter build with no adapter, unaffected by any of this). The Worker also auto-provisions a `SESSION` KV namespace on first deploy (Astro's Cloudflare adapter always wires one up for its Sessions feature, even though nothing on the site uses `Astro.session` yet) — negligible cost on Cloudflare's free tier, but worth knowing it exists before wondering where it came from.
