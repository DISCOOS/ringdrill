...
Because a `Person` is station-owned, `personRef` ties the roleplay to a station: the author selects the **Post** first and then picks (or creates) the `Person` from that station, so `personRef` is scoped by the chosen station. There are no scenario-less ("dangling") roleplays: every marker portrays a scenario person, so identity always flows through the Person and the marker's edit always reaches the brief.

**Post first, then Person.** The author chooses the marker's Post before any identity field is active; until then the identity card, position and description sections are disabled. With a Post chosen, the author either picks an existing station `Person` or creates one inline — a "+ Ny person" entry in the person picker opens the Person form. Identity fields are overrides of the selected Person, so there is nothing to override (and no "Tilpass" panel) until a Person is selected. `personRef` remains an editor-level invariant for a saved roleplay, satisfied by an explicit choice.

*Amended 2026-07-10:* the original decision auto-created the `Person` from whatever identity the author typed, so `personRef` needed no explicit step and creation order was free. In practice that produced empty, slug-only placeholder persons and let a marker be built before its Post was chosen. The flow is now Post-first with explicit person selection or inline creation. The denormalized effective-identity and write-back mechanisms below are unchanged.
