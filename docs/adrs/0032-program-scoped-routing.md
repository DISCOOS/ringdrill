...
## Addendum (2026-07-25): route namespace renamed `/program/` → `/plan/`

The `Program` → `Plan` Dart model rename ([tracked separately from DESIGN-010](../design/010-inline-preview-and-resolve-scope.md), see `docs/prompts/refactor-program-to-plan-and-person-description.md`) carries the in-app route prefix along with it: every path this ADR specifies as `/program/:uuid/...` is now `/plan/:uuid/...`, and the constants/helpers described above are renamed to match (`routeProgram` → `routePlan`, `programPath` → `planPath`, `programSegmentPath` → `planSegmentPath`, `programMapPath` → `planMapPath`, `programRosterPath` → `planRosterPath`, `programSegmentDefaultSlug`/`programSegmentFromSlug` → `planSegmentDefaultSlug`/`planSegmentFromSlug`). `ProgramService`/`ProgramView`/`_ProgramSegmentSwitcher` etc. above are likewise now `PlanService`/`PlanView`/`_PlanSegmentSwitcher`.

No permanent external links existed against the old `/program/` prefix (the in-app routes are client-only, never in AASA, the apex proxy, or the deep-link handler), so this is a clean rename rather than a new back-compat redirect layer: the *Back-compatibility* section above still applies to the pre-ADR-0032 un-prefixed paths, but the `/program/` → `/plan/` prefix change itself needed no redirect. The brief route this ADR realigns (`/program/:uuid/brief`) is `/plan/:uuid/brief`; the *external* `/brief/program/:uuid` redirect (interim, ADR-0041) moved to `/brief/plan/:uuid` in the same change — see the addendum on [ADR-0039](./0039-site-pwa-api-origins.md).

The `.drill` archive format is unaffected: the archive root manifest stays named `program.json` (ADR-0007) and its JSON keys are unaffected by the Dart class rename.
