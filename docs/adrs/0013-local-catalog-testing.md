...
## Addendum (2026-07-02): the CLI itself needed the same fix `AppConfig` already had

The "Note on `netlify dev` vs `netlify functions:serve`" section above states "`DrillClient` already calls `/.netlify/functions/*` directly for the upload/head/admin/market-feed paths, so those all work." That is only true for the **app's** `DrillClient` instances, built through `_buildCatalogClient()` / `AppConfig.functionsBasePathFor(baseUrl)` (which resolves to `/.netlify/functions` for a local base URL, `/api` otherwise). It was never true for the standalone CLI: `bin/ringdrill.dart` built its `DrillClient` with no `functionsBasePath` override, so it always used the class's own default, `/api` — meaning `make catalog-seed` and `make catalog-seed-demos` 404'd against `functions:serve` the whole time this ADR's workflow has existed. This had gone unnoticed because manual verification during earlier catalog work used a direct `curl` against `/.netlify/functions/drills-upload` instead of the Makefile target.

`bin/ringdrill.dart` cannot import `lib/utils/app_config.dart` to reuse `functionsBasePathFor` — that file imports `package:flutter/foundation.dart` for `kDebugMode`, which would violate [ADR-0005](./0005-cli-must-remain-flutter-free.md)'s Flutter-free rule for the CLI. Instead, the CLI gained its own `--functions-base-path` option (env `RINGDRILL_FUNCTIONS_BASE_PATH`, default `/api`, passed straight to `DrillClient`), mirroring the existing `--base-url`/`RINGDRILL_BASE_URL` pattern rather than auto-detecting "looks local" the way `AppConfig` does — the CLI has no compile-time debug/release split to gate an auto-detection branch on, so an explicit flag is the honest option. `catalog-seed`, `catalog-seed-demos` and `catalog-feed` now pass `RINGDRILL_FUNCTIONS_BASE_PATH=$(LOCAL_FUNCTIONS_BASE_PATH)` (default `/.netlify/functions`), and a new `netlify-dev-catalog-seed-all` target starts the backend, waits for it to come up, runs both seed targets, and stays attached to the server so a contributor no longer needs two shells and a manual `curl` workaround just to get a populated local catalog.
