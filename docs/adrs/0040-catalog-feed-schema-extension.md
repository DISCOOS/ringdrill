...
## Addendum (2026-07-02): `/catalog` language filter becomes a static list (supersedes "populated only with languages present in the current result set")

The `languageCode` addendum above states the filter's `<select>` is "populated only with the languages actually present in the current result set... not a fixed list." Reversed: the filter now always lists every language ADR-0007 supports (`nb`, `en`, sourced from `site/src/lib/languages.ts`'s `LANGUAGE_NAMES`), regardless of whether any currently-published plan actually uses each one.

**Why.** The data-driven version meant the entire control vanished whenever fewer than two distinct `languageCode` values existed in the current feed — which, before any plan had a language set, was always. A missing control reads as "this feature doesn't exist" or "this is broken," not "nothing to filter yet" — worse than a control that sometimes narrows to zero results. `distinctLanguageCodes()` (`site/src/lib/languages.ts`) is removed as dead code; nothing computes it anymore.

**Consequences.** Selecting a language with no matching plans yet is a legitimate, honest empty state (a filter that visibly does nothing when there is nothing to show, not a sign of a broken control) — same "the grid can end up showing nothing" case the existing `catalog__empty` handling was already built for. The filter option list is now identical to the language picker's own option list in `lib/views/program_form_screen.dart`, extended in lockstep the same way display names already were.
