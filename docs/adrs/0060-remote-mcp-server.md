...
> **All three now exist, and the third had to be built rather than declared.** The size cap and the timeout shipped with the endpoint. The rate limit did not, and was still missing after the endpoint went live: 60 tool calls per minute per caller, enforced in the function (`netlify/functions/lib/mcp-rate-limit.js`).
>
> Netlify's two declarative forms were both tried first and neither works here. A `rateLimit` in the function's `config` export and a `[redirects.rate_limit]` block in `netlify.toml` are read during a deploy's **post-processing** stage, and this site deploys with `netlify deploy --prod --dir=. --functions=…`, which never runs it. Netlify's own config resolver parses both without complaint and the artefact carries the translated rule, so everything upstream of the deploy looks correct; the deploy log simply ends at "Deploy is live!" with no post-processing section. Production proved it twice, once per form: 100 requests in under four seconds, 100 answers, no 429. Worth recording because the failure is silent in exactly the way a stale compiler bundle is — the configuration is valid, the tests are green, and the protection does not exist.
>
> Enforcing in the function has one advantage beyond working: it covers `/.netlify/functions/mcp`, which Netlify serves whether or not a redirect names it, and which a redirect-level rule could never have reached. It meters tool calls only: `initialize`, `tools/list`, `resources/read` and `search_catalog` stay free, so a client can always discover the server, and the exempt list is an allowlist so a tool added later is metered by default.
>
> **The counter is a plain read-modify-write, and the limit is therefore approximate.** Blobs has no atomic increment, so exactness needs a compare-and-swap on an ETag — the pattern `shared.js` uses for the slug index. That was attempted twice and took the endpoint down twice, because `onlyIfMatch` is not usable on this store. First with the ETag from `getWithMetadata`, which `set` does not compare against: every write after the first failed, the counter sat at 1, and nothing was enforced while everything looked healthy. Then with `getMetadata`, plus logic to distinguish real contention from a rejected condition — the ETag differs between reads here, so the retries exhausted and the branch that refuses on contention began refusing the second metered call of every session. Both were live before they were understood.
>
> What replaced it accepts undercounting under concurrency and keeps the bound on a client looping, which is what a runaway script and a stuck agent both are. The structural property is that **no store failure can produce a refusal**: there is one refusal path and it fires only on a value successfully read. That is not a preference, it is the invariant both outages violated.
>
> Three verification lessons, in the order they were learned. A test of the artefact cannot prove behaviour that only exists after a deploy — the first attempt shipped green while asserting the bundler's `trafficRules`, one layer short of registration. A fake that offers methods the code under test does not call proves nothing: the endpoint suite implemented `getWithMetadata` throughout, so every metered call ran the fail-open path and a 429 assertion could not have failed. And a test that filters out the degraded verdicts is measuring only the calls that worked — the old concurrency test counted the CAS winners and never noticed the losers being waved through. All three were green while the feature did not exist.
