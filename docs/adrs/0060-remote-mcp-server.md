...
>
> Enforcing in the function has one advantage beyond working: it covers `/.netlify/functions/mcp`, which Netlify serves whether or not a redirect names it, and which a redirect-level rule could never have reached. The counter is an ETag compare-and-swap on Netlify Blobs, because Blobs has no atomic increment and a read-modify-write lets concurrent callers overshoot — the case a limit exists for. It meters tool calls only: `initialize`, `tools/list`, `resources/read` and `search_catalog` stay free, so a client can always discover the server, and the exempt list is an allowlist so a tool added later is metered by default. It fails open, from store construction onward, on the grounds that a limiter which can take the endpoint down is worse than the abuse it prevents while no tool touches private state.
>
> The verification lesson is the durable part. The first attempt shipped with a green test asserting the bundler's `trafficRules` output, one layer short of registration, so it passed throughout while nothing was enforced. Behaviour that only exists after a deploy cannot be proven by a test of the artefact. In-function enforcement is testable offline — including the contention path — which is itself an argument for it (`netlify/tests/mcp-rate-limit.test.mjs`).
