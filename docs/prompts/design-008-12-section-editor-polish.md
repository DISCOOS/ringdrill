...
1. **No duplicated section title.** The rail already shows the selected section highlighted, so **drop the section-title heading in the detail pane**. Also drop the `‹ ›` previous/next arrows on wide (the rail is the navigation). Keep only the `⋮` section-actions menu, aligned to the top-right of the detail pane.
