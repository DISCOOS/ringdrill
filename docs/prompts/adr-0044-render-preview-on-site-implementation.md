...
**`og:image` is mandatory and load-bearing.** Without it Messenger/Facebook render only the bare domain with no card. Emit the same image tags the function now ships: `og:image`, `og:image:secure_url`, `og:image:type=image/png`, `og:image:width=1200`, `og:image:height=630`, `og:image:alt`, plus `twitter:card=summary_large_image`, `twitter:title`, `twitter:description`, `twitter:image`. Point them at the shared static card `https://ringdrill.app/og-default.png`. That asset already exists at the apex root — committed to both `web/og-default.png` (served by the PWA today) and `site/public/og-default.png` (served after the ADR-0039 apex migration), so `/og-default.png` resolves in both topologies. Reuse it; do not regenerate a different image.

**Pitfall — the route MUST win over the static fallback and override the site's default OG head.** Two related traps. First, the current static site serves `index.html` for unknown paths (SPA/catch-all in `site/public/_redirects`), so before this route exists `/i/<slug>` silently returns the home page with the site's generic OG (`htmlTitle` "RingDrill: Rullering uten regneark", the marketing `metaDescription`, `og:image = /brand/logo.png`, a 512×512 square that crops badly). The new on-demand `/i/[slug]` route must take precedence over that fallback. Second, do not inherit `BaseLayout`'s default OG: pass the per-plan `og:title` (`${name} · RingDrill`), per-plan `og:description`, and `og:image = /og-default.png` (1200×630) into the layout so they win. Absolute URLs matter: `og:url`/`canonical`/`og:image` must be the production apex (`https://ringdrill.app/...`), not the Cloudflare `*.pages.dev` preview host. Confirm in the Facebook Sharing Debugger that `og:title` is the plan name, `og:description` is the plan description, and `og:image` is `og-default.png` — not `logo.png`.
