...
* Run `make mcp-bundle` after changing anything under `lib/data/source/`, `lib/models/`, `lib/services/brief/` or `lib/l10n/`. Those are cross-compiled to JavaScript for the hosted MCP endpoint (ADR-0060) and the bundle is committed; a stale one serves old compiler behaviour from `/mcp` while the app and CLI look fine. Changing how the function is *packaged* (`netlify.toml`, `netlify/functions/mcp.js`, `netlify/functions/lib/`, `mcp/tools.mjs`) is a different risk with the same silent failure mode — run `node --test netlify/tests/mcp-packaging.test.mjs`, and `npm run smoke:mcp` once it is deployed. See rule 1 in `AGENTS.md`.
