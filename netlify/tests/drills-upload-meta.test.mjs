...
// resolvePublishPolicy still derives a *default* from ownerId, but it is no
// longer what decides who may write. That moved to lib/authorize.js, where
// ADR-0025's matrix runs against a verified principal before OCC — see
// authorize.test.mjs, which replaced the note that used to live here warning
// that this value was a label rather than a gate.
//
// The remaining contract is narrow: given an ownerId that authorization has
// already settled, what policy does a *new* plan start at?
test("resolvePublishPolicy: derives the starting policy for a new plan, nothing more", () => {
    assert.deepEqual(resolvePublishPolicy({ ownerId: "anon" }), { author: "anon", accessPolicy: "public" });
    assert.deepEqual(resolvePublishPolicy({ ownerId: "acc-42" }), { author: "acc-42", accessPolicy: "account" });
});
