...
void main() {
  group('Stage 1b writer', () {
    test('writes Stage 1b markdown fields as .md files in archive', () {
      ...
      expect(
        byName['exercises/ex-1/stations/0/critical-questions.md'],
        'crit-0',
      );
      expect(byName['exercises/ex-1/stations/0/leader-answers.md'], 'leader-0');
      expect(
        byName['exercises/ex-1/stations/0/director-notes.md'],
        'director-0',
      );
      ...
      expect(
        exJson.keys.any((k) => k.endsWith('Md')),
        isFalse,
      ...
      expect(
        rpJson.keys.any((k) => k.endsWith('Md')),
        isFalse,
    ...
    test(
      'writes templateId and variantSuffix into JSON manifests (not .md)',
      () {
        ...
        expect(
          byName.containsKey('exercises/ex-1/stations/0/variantSuffix.md'),
          isFalse,
        );
      },
    );
  ...
  group('Stage 1b reader', () {
    test('reads back Stage 1b markdown fields from archive (roundtrip)', () {
      ...
      expect(decoded.beforeRoundMd, 'before-round');
    ...
    test(
      'Stage 1b empty md file roundtrips as empty string, missing as null',
      () {
  ...
  group('Stage 1b content hash', () {
    test('includes new markdown fields', () {
      ...
      // beforeRoundMd
      final hBefore = base
          .copyWith(beforeRoundMd: 'changed')
          .computeContentHash();
      expect(hBefore, isNot(h1), reason: 'beforeRoundMd must affect hash');
      ...
      final modEx = _exercise().copyWith(stations: [modStation, _station(1)]);
      ...
      final modEx2 = _exercise().copyWith(stations: [modStation2, _station(1)]);
      ...
      expect(
        withRpProp.computeContentHash(),
        isNot(withRp.computeContentHash()),
        reason: 'propsMd must affect hash',
      );

      // templateId and variantSuffix (structural, always in JSON)
      final withTemplate = base.copyWith(
        exercises: [_exercise(templateId: 'tmpl')],
      );
      expect(
        withTemplate.computeContentHash(),
        isNot(h1),
      ...
      final modVariant = _exercise().copyWith(
        stations: [
          _station(0).copyWith(variantSuffix: 'Alpha'),
          _station(1),
        ],
      );
      final withVariant = base.copyWith(exercises: [modVariant]);
      expect(
        withVariant.computeContentHash(),
        isNot(h1),
    ...
    test(
      ...
      () {
        ...
        final ex2 = Exercise(
          ...
          evaluationTime: 5,
          rotationTime: 5,
          stations: [const Station(index: 0, name: 'S0', situationMd: 'sit0')],
          schedule: const [],
          methodMd: 'method-z',
        );
