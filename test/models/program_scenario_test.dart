...
void main() {
  ...
  group('backward compatibility', () {
    ...
    test('opens a synthetic pre-existing archive with no locations/persons/'
      ...
      final drillFile = DrillFile.fromProgram(program, 'legacy');
  ...
  group('content hash sensitivity', () {
    ...
    Program programWithStation(Station station) => base().copyWith(
          exercises: [
            Exercise(
              uuid: 'ex-1',
              name: 'Ex',
    ...
    test('changes when a station gains a location', () {
      final prog = programWithStation(stationWith());
      final changed = programWithStation(stationWith(locations: const [lkp]));
      expect(prog.computeContentHash(), isNot(changed.computeContentHash()));
    });

    test('changes when a location field changes', () {
      final prog = programWithStation(stationWith(locations: const [lkp]));
      final changed = programWithStation(
    ...
    test('changes when a station gains a person', () {
      final prog = programWithStation(stationWith());
      final changed = programWithStation(stationWith(persons: const [anne]));
      expect(prog.computeContentHash(), isNot(changed.computeContentHash()));
    });

    test('changes when a person field changes', () {
      final prog = programWithStation(stationWith(persons: const [anne]));
      final changed = programWithStation(
