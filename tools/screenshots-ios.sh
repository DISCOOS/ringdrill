...
#
# Commands:
#   tools/screenshots.sh ios devices            List available simulators.
#   tools/screenshots.sh ios lang <nb|en>       Set the booted sim's language and
#                                           reboot it (run before `flutter run`).
#   tools/screenshots.sh ios prep               Clean status bar: 09:41, full battery,
#                                           full signal, no carrier name.
#   tools/screenshots.sh ios shot <lang> <name> Capture the booted sim to
#                                           store/screenshots/ios/<class>/<lang>/<name>.png
#                                           (<class> = iphone|ipad, auto-detected).
#   tools/screenshots.sh ios appearance <light|dark>
...
#   tools/screenshots.sh ios prep
#   tools/screenshots.sh ios shot en 01-schedule
#   tools/screenshots.sh ios shot en 02-map
#   tools/screenshots.sh ios appearance dark        # 03-live looks best in dark
...
cmd_lang() {
  ...
  echo "done."
  echo "Next:  flutter run, import tools/screenshots/demo-$file.drill into the app, then:  tools/screenshots.sh ios prep"
}
...
cmd_prep() {
  ...
  echo "Next:  tools/screenshots.sh ios shot $lang 01-schedule   (light mode)"
}
...
cmd_appearance() {
  ...
  local lang; lang="$(cat "$STATE_LANG" 2>/dev/null || echo '<lang>')"
  case "$mode" in
    dark)  echo "Next:  tools/screenshots.sh ios shot $lang 03-live" ;;
    light) echo "Next:  tools/screenshots.sh ios shot $lang 04-brief" ;;
  esac
}
...
cmd_shot() {
  ...
  case "$name" in
    01-schedule) echo "Next:  tools/screenshots.sh ios shot $lang 02-map" ;;
    02-map)      echo "Next:  tools/screenshots.sh ios appearance dark   (then: shot $lang 03-live)" ;;
    03-live)     echo "Next:  tools/screenshots.sh ios appearance light  (then: shot $lang 04-brief)" ;;
    04-brief)    echo "Pass complete ($class/$lang). Next pass: 'tools/screenshots.sh ios lang $other' for the other language, or boot the iPad Pro 13-inch simulator for the iPad set, then start again from 'lang'." ;;
    *)           echo "Next:  continue with the remaining shots, or 'tools/screenshots.sh ios lang <nb|en>' for the next pass." ;;
  esac
}

usage() {
  cat <<'EOF'
tools/screenshots.sh ios — capture App Store screenshots from the iOS Simulator.
...
TYPICAL PASS (iPhone, English; repeat for nb and for the iPad)
  ...
  tools/screenshots.sh ios shot en 01-schedule
  tools/screenshots.sh ios shot en 02-map
  tools/screenshots.sh ios appearance dark
  tools/screenshots.sh ios shot en 03-live
  tools/screenshots.sh ios appearance light
  tools/screenshots.sh ios shot en 04-brief
